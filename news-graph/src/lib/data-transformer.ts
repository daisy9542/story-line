import { EventNode, GraphNode, NodeType, RelatedEventNode, GraphEdge } from "@/types";
...
export function transformDataToGraph(data: any) {
  ...
  if (data.entities && Array.isArray(data.entities)) {
    data.entities.forEach((entity: any, index: number) => {
      ...
      if (entity.type === "assets" && entity.priceChange !== undefined) {
        const priceChangeText = entity.priceChange > 0
          ? `+${entity.priceChange}%`
          : `${entity.priceChange}%`;
  ...
  // 3. 添加历史类似事件节点和边
  if (data.historical_similar_events && Array.isArray(data.historical_similar_events)) {
    data.historical_similar_events.forEach((event: any, index: number) => {
      ...
      const histEventNode: EventNode = {
        ...
        // 添加标签显示相似原因
        tags: event.similarity_reason ? [`相似原因: ${event.similarity_reason}`] : undefined,
      };
