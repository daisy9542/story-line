...
export default function NewsEvents({ newsEvents }: { newsEvents: NewsEvent[] }) {
  ...
  const [activeTab, setActiveTab] = useState<'causal' | 'timeline' | 'historical' | 'citations' | 'entities'>('causal');
  ...
  const TabButton = ({ id, label, icon, active }: { id: typeof activeTab; label: string; icon: React.ReactNode; active: boolean }) => (
    <button
                ...
                {activeTab === 'causal' && selectedEvent.causal_inferences && (
                  <div className="flex flex-col gap-4">
                    {selectedEvent.causal_inferences.map((analysis, index) => (
                      <div
                        key={`causal-${focusedEventId}-${index}`}
                    ...
                    {selectedEvent.historical_comparisons.map((comparison, index) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <p className="text-sm text-slate-400/80">{comparison.event_date || selectedEvent.analysis_date}</p>
